...
get_capabilities(DBusConnection *session_bus) {
    // 60ms is too short: the call is async, but during startup the event loop
    // may not pump DBus within 60ms, expiring the pending reply client-side.
    return glfw_dbus_call_method_with_reply(session_bus, NOTIFICATIONS_SERVICE, NOTIFICATIONS_PATH, NOTIFICATIONS_IFACE, "GetCapabilities", 5000, got_capabilities, NULL, DBUS_TYPE_INVALID);
}
